...
/* eslint-disable */
function checkDeploymentStatus(deployment, token, version, teamId, debug, apiUrl) {
    return __asyncGenerator(this, arguments, function* checkDeploymentStatus_1() {
        ...
        while (true) {
            if (!allBuildsCompleted) {
                const buildsData = yield __await(utils_1.fetch(`${apiDeployments}/${deployment.id}/builds${teamId ? `?teamId=${teamId}` : ''}`, token, { apiUrl }));
                ...
                if (ready_state_1.isReady(deploymentUpdate) && !readyEventFired) {
                    debug('Deployment state changed to READY 2');
                    readyEventFired = true;
                    yield yield __await({ type: 'ready', payload: deploymentUpdate });
                }
                if (ready_state_1.isAliasAssigned(deploymentUpdate)) {
                    debug('Deployment alias assigned');
                    return yield __await(yield yield __await({ type: 'alias-assigned', payload: deploymentUpdate }));
                }
                const aliasError = ready_state_1.isAliasError(deploymentUpdate);
                if (ready_state_1.isFailed(deploymentUpdate) || aliasError) {
                    debug(aliasError
