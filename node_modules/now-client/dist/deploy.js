...
function createDeployment(metadata, files, options, debug) {
    return __asyncGenerator(this, arguments, function* createDeployment_1() {
        ...
        const apiDeployments = utils_1.getApiDeploymentsUrl(metadata);
    ...
    return __asyncGenerator(this, arguments, function* deploy_1() {
        ...
        if (deployment) {
            if (ready_state_1.isReady(deployment) && ready_state_1.isAliasAssigned(deployment)) {
                ...
                yield yield __await({ type: 'ready', payload: deployment });
                debug('Deployment alias assigned');
                return yield __await(yield yield __await({ type: 'alias-assigned', payload: deployment }));
            }
            try {
                debug('Waiting for deployment to be ready...');
                try {
                    for (var _e = __asyncValues(deployment_status_1.default(deployment, options.token, metadata.version, options.teamId, debug, options.apiUrl)), _f; _f = yield __await(_e.next()), !_f.done;) {
